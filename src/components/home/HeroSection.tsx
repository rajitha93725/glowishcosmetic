...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-[#fff0f5]">
      {/* Decorative blobs */}
      <div className="absolute -top-32 -right-32 w-[500px] h-[500px] rounded-full bg-pink-200/50 blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 -left-24 w-[400px] h-[400px] rounded-full bg-pink-300/30 blur-3xl pointer-events-none" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-rose-100/40 blur-3xl pointer-events-none" />

      <div className="relative max-w-7xl mx-auto px-6 pt-20 pb-10 min-h-screen flex flex-col justify-center">
        {/* Top label */}
        <div className="flex items-center gap-3 mb-8 animate-fade-in">
          <div className="h-px w-10 bg-pink-400" />
          <span className="text-pink-500 tracking-[0.3em] uppercase text-xs font-semibold">
            New Collection 2024
          </span>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          {/* Left: text */}
          <div>
            <h1 className="font-display text-[clamp(3.5rem,8vw,7rem)] font-bold leading-[0.92] tracking-tight text-pink-900 mb-8">
              Beauty<br />
              Made to{" "}
              <span
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <Link
                href="/shop"
                className="group relative overflow-hidden bg-pink-500 text-white font-bold px-8 py-4 rounded-full text-sm tracking-wide transition-all duration-300 hover:shadow-xl hover:shadow-pink-300/50 hover:-translate-y-0.5"
              >
            ...
            {/* Stats */}
            <div className="flex gap-8 mt-14 pt-8 border-t border-pink-100">
              {[["2K+", "Happy Customers"], ["150+", "Products"], ["100%", "Clean Formula"]].map(([n, l]) => (
                <div key={l}>
                  <p className="font-display text-2xl font-bold text-pink-600">{n}</p>
